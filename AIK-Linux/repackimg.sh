...
echo " ";
case $imgtype in
  AOSP) bin/$arch/mkbootimg --kernel "$kernel" --ramdisk "$ramdisk" $second --cmdline "$cmdline" --board "$board" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff --second_offset "$secondoff" --tags_offset "$tagsoff" --os_version "$osver" --os_patch_level "$oslvl" $dtb -o boot.img;;
...
if [ "$imgtype" = "CHROMEOS" ]; then
  ...
  echo " ";
  bin/$arch/futility vbutil_kernel --pack boot.img --keyblock bin/chromeos/kernel.keyblock --signprivate bin/chromeos/kernel_data_key.vbprivk --version 1 --vmlinuz unsigned-new.img --bootloader bin/chromeos/empty --config bin/chromeos/empty --arch arm --flags 0x1;
  if [ ! $? -eq "0" ]; then
    abort;
    exit 1;
  fi;
