	...
	unsigned int relation)
{
	...
	unsigned int freq_turbohi;
	unsigned int index_turbohi;
	unsigned int freq_turbomd;
	unsigned int index_turbomd;
	unsigned int freq_turbolo;
	unsigned int index_turbolo;
	unsigned int freq_middle;
	unsigned int index_middle;
	unsigned int freq_normal;
	unsigned int index_normal;
	//unsigned int freq_low, index_low;
	//unsigned int freq_lowest, index_lowest;
	unsigned int freq_sleepingbeauty;
	unsigned int index_sleepingbeauty;
	int cpu;
	int cur;
	int min;
	//min = policy->min;
	...
	cpu = policy->cpu;
	cur = policy->cur;
	old_mode = readl(ADDR_CLKPWR_CLK_ARMAHB_MODE);
	// 312MHZ
	arm11_freq[0] = (312U * 1000);
	// 416MHZ
	//arm11_freq[1] = FREQ_MHZ(416);
	// 520MHZ
	//arm11_freq[2] = FREQ_MHZ(520);
	// 624MHZ
	arm11_freq[1] = (624U * 1000);
	// 754MHZ
	arm11_freq[2] = (754U * 1000);
	// 832MHZ
	arm11_freq[3] = (832U * 1000);
	// 1040MHZ
	arm11_freq[4] = (910U * 1000);
	// 1248MHZ
	arm11_freq[5] = (1040U * 1000);

	min = arm11_freq[0];
	max = arm11_freq[5];

	index_turbohi	= info->index_turbohi;
	freq_turbohi 	= info->freq_tbl[index_turbohi].cpu_freq * 1000;
	index_turbomd		= info->index_turbomd;
	freq_turbomd		= info->freq_tbl[index_turbomd].cpu_freq * 1000;
	index_turbolo		= info->index_turbolo;
	freq_turbolo		= info->freq_tbl[index_turbolo].cpu_freq * 1000;
	index_middle	= info->index_middle;
	freq_middle	= info->freq_tbl[index_middle].cpu_freq * 1000;
	index_normal	= info->index_normal;
	freq_normal	= info->freq_tbl[index_normal].cpu_freq * 1000;
	//index_low	= info->index_low;
	//freq_low	= info->freq_tbl[index_low].cpu_freq * 1000;
	index_sleepingbeauty	= info->index_sleepingbeauty;
	freq_sleepingbeauty	= info->freq_tbl[index_sleepingbeauty].cpu_freq * 1000;
	//index_lowest	= info->index_lowest;
	//freq_lowest		= info->freq_tbl[index_lowest].cpu_freq * 1000;
	...
	/*we support only two modes  - 0xC & 0xF*/
	/****** is that so? lol ******/
	if (freqs.new == arm11_freq[0])
	{
		new_mode = 0x0A;
	}
	else if (freqs.new == arm11_freq[1])
	{
		new_mode = 0x0B;
	}
	else if (freqs.new == arm11_freq[2])
	{
		new_mode = 0x0C;
	}
	else if (freqs.new == arm11_freq[3])
	{
		new_mode = 0x0D;
	}
		else if (freqs.new == arm11_freq[4])
	{
		new_mode = 0x0E;
	}
	else if (freqs.new == arm11_freq[5])
	{
		new_mode = 0x0F;
	}
	/*else if (freqs.new == arm11_freq[6])
	{
		new_mode = 0x0E;
	}
	else if (freqs.new == arm11_freq[7])
	{
		new_mode = 0x0F;
	}
	*/
	...
	if (freqs.new == freqs.old) {
		pr_info("%s: new mode is the same: %x <-> %x\n", __func__, old_mode, new_mode);
		pr_info("%s: new cpu freq is the same: %u <-> %u\n",
			 __func__, freqs.old, freqs.new);
		return ret;
	}
	else if (freqs.new > freqs.old || freqs.new < freqs.old) {
		pr_info("debug: %x %x %x ", readl(ADDR_CLKPWR_CLK_APPSPLL_DIVIDERS), readl(ADDR_CLKPWR_CLK_APPSPLL_FRAC_DIVIDERS), readl(ADDR_CLKPWR_CLK_APPSPLL_MODE));
		pr_info("%s: mode change: %x --> %x\n", __func__, old_mode, new_mode);
		pr_info("%s: cpu freq change: %u --> %u\n", __func__,
			freqs.old, freqs.new);

			if (freqs.new > max)
				return freqs.new = freq_turbohi;
			if (freqs.new < min)
				return freqs.new = freq_sleepingbeauty;

			/* Height Frequencies Need's special hundling :) */
			/* lol let me handle them for you ~watson */
			if (freqs.new == freq_turbohi || freqs.new == freq_turbomd || freqs.new == freq_turbolo || freqs.new == freq_normal || freqs.new == freq_middle)
			{
			...
			else if (freqs.new == freq_sleepingbeauty)
			{
