...
static int turbo_down_threshold = 25;
module_param_named(turbo_down_threshold, turbo_down_threshold, int, S_IRUGO);
static int turbo_up_threshold = 80;
module_param_named(turbo_up_threshold, turbo_up_threshold, int, S_IRUGO);
...
static inline void cpufreq_bcm_turbo_on(void)
{
	...
	if (bcm_cpufreq_gov_info.cur_policy)
		cpufreq_driver_target(bcm_cpufreq_gov_info.cur_policy,
				      plat->freq_turbomd, CPUFREQ_RELATION_H);
}
	...
	unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		 ...
		 */
		if (dvfs_disable == 0)
			__cpufreq_driver_target(policy, plat->freq_turbomd,
						CPUFREQ_RELATION_H);
		else
			__cpufreq_driver_target(policy, plat->freq_turbomd,
						CPUFREQ_RELATION_H);
