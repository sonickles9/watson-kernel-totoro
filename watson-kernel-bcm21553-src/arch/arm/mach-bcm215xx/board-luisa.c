...
*
*	@file	arch/arm/mach-bcm215xx/board-luisa.c
*
...
#define CONFIG_CAM_CPI //BYKIM For LUISA
...
#define KEY_3POLE_THRESHOLD	650	// ~ 55 and 57 ~, measured on Totoro
#define KEY1_THRESHOLD_L	0	// 9 to 83, mesaured on Luisa
#define KEY1_THRESHOLD_U	108
#define KEY2_THRESHOLD_L	109	// 117 to 183, mesaured on Luisa
#define KEY2_THRESHOLD_U	228
#define KEY3_THRESHOLD_L	229	// 202 to 314, mesaured on Luisa
...
static struct bcm_keymap newKeymap[] = {
	...
	{BCM_KEY_ROW_0, BCM_KEY_COL_6, "Volume Up", KEY_VOLUMEUP},
	...
	{BCM_KEY_ROW_1, BCM_KEY_COL_6, "Volume Down", KEY_VOLUMEDOWN},
	...
	{BCM_KEY_ROW_2, BCM_KEY_COL_6, "otp_ok", KEY_OK},
	{BCM_KEY_ROW_2, BCM_KEY_COL_7, "unused", 0},
	{BCM_KEY_ROW_3, BCM_KEY_COL_0, "L", KEY_L},
	{BCM_KEY_ROW_3, BCM_KEY_COL_1, "key backspace", KEY_BACKSPACE},
	{BCM_KEY_ROW_3, BCM_KEY_COL_2, "key shift", KEY_LEFTSHIFT},
	...
	{BCM_KEY_ROW_4, BCM_KEY_COL_4, "key question", KEY_QUESTION},
	{BCM_KEY_ROW_4, BCM_KEY_COL_5, "key enter", KEY_ENTER},
	{BCM_KEY_ROW_4, BCM_KEY_COL_6, "unused", 0},
	{BCM_KEY_ROW_4, BCM_KEY_COL_7, "unused", 0},
	{BCM_KEY_ROW_5, BCM_KEY_COL_0, "key alt", KEY_RIGHTALT},
	{BCM_KEY_ROW_5, BCM_KEY_COL_1, "sym", KEY_CANCEL},
	{BCM_KEY_ROW_5, BCM_KEY_COL_2, "0", KEY_0},
	{BCM_KEY_ROW_5, BCM_KEY_COL_3, "key space1", KEY_SPACE},
	{BCM_KEY_ROW_5, BCM_KEY_COL_4, "key space2", KEY_SPACE},
	{BCM_KEY_ROW_5, BCM_KEY_COL_5, "key dot", KEY_DOT},
	{BCM_KEY_ROW_5, BCM_KEY_COL_6, "unused", 0},
	{BCM_KEY_ROW_5, BCM_KEY_COL_7, "unused", 0},
	{BCM_KEY_ROW_6, BCM_KEY_COL_0, "key mute", KEY_HOMEPAGE},
	{BCM_KEY_ROW_6, BCM_KEY_COL_1, "key mail", KEY_MAIL},
	{BCM_KEY_ROW_6, BCM_KEY_COL_2, "key menu", KEY_MENU},
	{BCM_KEY_ROW_6, BCM_KEY_COL_3, "key home", KEY_HOME},
	{BCM_KEY_ROW_6, BCM_KEY_COL_4, "key back", KEY_BACK},
	{BCM_KEY_ROW_6, BCM_KEY_COL_5, "search", KEY_SEARCH},
...
#ifdef CONFIG_KEYPAD_BACKLIGHT
struct platform_device bcm_keypad_led_devices = {
	.name = "keypad-led", 
	.id = 0,
};
static struct platform_keypad_led_data bcm_keypad_led_data = {
	.max_brightness = 255,
...
struct i2c_slave_platform_data luisa_cam_pdata = {
        .i2c_spd = I2C_SPD_400K,
};

static struct i2c_board_info __initdata bcm21553_cam_i2c_board_info[] = {
        {I2C_BOARD_INFO("cami2c", 0x1a),
         .platform_data = (void *)&luisa_cam_pdata,
         .irq = IRQ_CAM_CCP2, 
         },
};
#endif
...
//	static u16 bat_adc[] = {0x2BE, 0x2D3, 0x2E8, 0x2FC, 0x311, 0x32C, 0x364};
	...
	static u32 TempAdcTable[] = {     747,695,642,592,516,464,395,350,305,267,214,185,146,132,113,104,99,91,}; 
...
#define SENSOR_SDA 28
#define SENSOR_SCL 31
...
static struct i2c_gpio_platform_data otp_i2c_gpio_data = {
        .sda_pin    = OTP_SDA,
        .scl_pin    = OTP_SCL,
        .udelay  = 3,  //// brian :3
        .timeout = 100,
...
static struct platform_device otp_i2c_gpio_device = {
        .name       = "i2c-gpio",
        .id     = 0x7,
        .dev        = {
                .platform_data  = &otp_i2c_gpio_data,
        },
};

static void otp_init(void)
{
	board_sysconfig(SYSCFG_OTP, SYSCFG_ENABLE);
}

module_init(otp_init);

#if defined(CONFIG_BRCM_HEADSET)  || defined(CONFIG_BRCM_HEADSET_MODULE)
#if defined(CONFIG_HEADSET_DET_GPIO)
...
static struct i2c_board_info __initdata athenaray_i2cgpio4_board_info[] = {
	...
	{
				I2C_BOARD_INFO("ofm", 0x53),
                    	.irq = GPIO_TO_IRQ(OTP_INT),
	},
...
int board_sysconfig(uint32_t module, uint32_t op)
{
	...
	case SYSCFG_RESETREASON_AP_ONLY_BOOT:
	 ...
	 case SYSCFG_CAMERA:
			...
			writel(readl(ADDR_SYSCFG_IOCR0)|SYSCFG_IOCR0_DIGMIC_GPIO6362_MUX , ADDR_SYSCFG_IOCR0);  //set gpio

			// GPIO direction (GPIO63 : ouptput)
                        val = readl(ADDR_GPIO_IOTR3);
                        val &= ~(3<<30);  //clear register
                        val |= (2<<30);     //output mode
                        writel(val, ADDR_GPIO_IOTR3);  //set direction

			// Set GPIO value to low
                        val = readl(ADDR_GPIO_GPOR1);
                        val &= ~(1<<31);// low
                        writel(val, ADDR_GPIO_GPOR1);  //set gpio value

			// Enable pull-up or pull-down (GPIO63)
                        val = readl(ADDR_GPIO_GPIPEN1);
                        val |= (1<<31);// enable pull-up or pull-down
                        writel(val, ADDR_GPIO_GPIPEN1);  //set use pull value

			// Set GPIO64 pull-down
	...
	case SYSCFG_SDHC3:
		if (op == SYSCFG_INIT) {
			...
			printk("SDIO3 Driving Strenght is 6mA\n");
			val = readl(ADDR_SYSCFG_IOCR4);
...
#define EXT_CTRL_REQ2	4
	...
	case SYSCFG_HAPTIC_VIB:
		if (op == SYSCFG_INIT) {
			...
			/* BIT12 */
			writel(readl(ADDR_SYSCFG_IOCR0) |
				SYSCFG_IOCR0_GPIO52_GPEN7_MUX, ADDR_SYSCFG_IOCR0);
			writel(readl(ADDR_SYSCFG_IOCR0) |
				SYSCFG_IOCR0_GPEN11_SPI_GPIO56_L_MUX |
	...
	case SYSCFG_SENSORS:
		if(op == SYSCFG_ENABLE){
                    ...
                    /*PROXI_INT =>GPIO29 IOCR0[8,7] state=000*/
                	writel(readl(ADDR_SYSCFG_IOCR0) & ~SYSCFG_IOCR0_GPIO2928_GPEN1312_MUX(1),  ADDR_SYSCFG_IOCR0);    

                    /*SENSOR_SCL =>GPIO31  IOCR5[3,2] state=000*/
			writel(readl(ADDR_SYSCFG_IOCR5) & ~SYSCFG_IOCR5_GPIO31_MUX(1),  ADDR_SYSCFG_IOCR5);	
			writel(readl(ADDR_GPIO_IOTR1) & ~(0x3 <<( SENSOR_SCL-16)*2 )  | (0x2 << ( SENSOR_SCL-16)*2) , ADDR_GPIO_IOTR1);

                    /*SENSOR_SDA => GPIO28 IOCR0[8,7] state=000*/
                	writel(readl(ADDR_SYSCFG_IOCR0) & ~SYSCFG_IOCR0_GPIO2928_GPEN1312_MUX(1),  ADDR_SYSCFG_IOCR0);                    
			writel(readl(ADDR_GPIO_IOTR1) & ~(0x3 <<( SENSOR_SDA-16)*2 )  | (0x2 << ( SENSOR_SDA-16)*2) , ADDR_GPIO_IOTR1);
       ...
       case SYSCFG_OTP:
		if(op == SYSCFG_ENABLE){
                    ...
                    /* OTP_POWER_EN => GPIO54   IOCR0[27,14] state=011*/
			writel(readl(ADDR_SYSCFG_IOCR0) |
				SYSCFG_IOCR0_GPEN9_SPI_GPIO54_L_MUX |
				SYSCFG_IOCR0_GPEN9_SPI_GPIO54_H_MUX, ADDR_SYSCFG_IOCR0);

                    /*OTP_INT=> GPIO22 IOCR6[5] state=000*/
            		writel(readl(ADDR_SYSCFG_IOCR6) & ~(0x1 << 5)  | (0x0 << 5)   , ADDR_SYSCFG_IOCR6);	
			
                    /*OTP_SHDN => GPIO35  IOCR5[11,10] state=000*/
            		writel(readl(ADDR_SYSCFG_IOCR5) & ~(0x3 << 10)  | (0x0 << 10)   , ADDR_SYSCFG_IOCR5);			            			

                    /* BSC3 : Broadcom Serial Controller3 - High-Speed Registers*/
                    /* IOCR3[31]  BSC3_EN : Enable the 3rd BSC on GPIO7(CLK), GPIO15(DATA)  0 - disabled, 1 - enabled*/
			writel(readl(ADDR_SYSCFG_IOCR3) & ~SYSCFG_IOCR3_I2C3_EN  , ADDR_SYSCFG_IOCR3);	

                    /*OTP_SCL => GPIO7 IOCR1[7]  state=000*/
			writel(readl(ADDR_SYSCFG_IOCR1) & ~(0x1<<7)  , ADDR_SYSCFG_IOCR1);	                    
                	writel(readl(ADDR_GPIO_IOTR0) & ~(0x3 << (OTP_SCL * 2))  | (0x2 << (OTP_SCL * 2)) , ADDR_GPIO_IOTR0);

                    /*OTP_SDA => GPIO15 IOCR1[15] state=000*/
			writel(readl(ADDR_SYSCFG_IOCR1) & ~(0x1<<15)  , ADDR_SYSCFG_IOCR1);	                    
			writel(readl(ADDR_GPIO_IOTR0) & ~(0x3 << (OTP_SDA * 2))  | (0x2 << (OTP_SDA * 2)) , ADDR_GPIO_IOTR0);   
       ...
       case SYSCFG_KEYLED:
		if(op == SYSCFG_ENABLE){
                    ...
                    /*KEY_LED_EN => GPIO62 IOCR0[24] state = 001*/
                	writel(readl(ADDR_SYSCFG_IOCR0) |SYSCFG_IOCR0_DIGMIC_GPIO6362_MUX , ADDR_SYSCFG_IOCR0);
			writel(readl(ADDR_GPIO_IOTR3) & ~(0x3 <<( 62-48)*2 )  | (0x2 << ( 62-48)*2) , ADDR_GPIO_IOTR3);        
		}
		else if (op == SYSCFG_INIT){
		/*[todo]Interrupt mode config*/
		}
...
#ifdef CONFIG_KEYPAD_BACKLIGHT
	bcm_keypad_led_devices.dev.platform_data = &bcm_keypad_led_data;
#endif
...
#define GPIPUD_PULL_DOWN(GPIO) (~(1<<(GPIO%32)))
				...
				/*Set as GPIO*/
				writel(readl(ADDR_SYSCFG_IOCR0)|(1<<29)&(~(1<<25)),ADDR_SYSCFG_IOCR0);/*45*/
				
				writel(readl(ADDR_SYSCFG_IOCR5)|(1<<24),ADDR_SYSCFG_IOCR5);/*58,59,60*/


				/*Set as input */
				writel(readl(ADDR_GPIO_IOTR2)&(~(3<<IOTR_GPIO(45))),ADDR_GPIO_IOTR2);
				...
				/*Enable pull up/down*/
				writel(readl(ADDR_GPIO_GPIPEN1)|GPIPEN_PULL_EN(45),ADDR_GPIO_GPIPEN1);
				...
				/*Set as pull down*/
				writel(readl(ADDR_GPIO_GPIPUD1)&GPIPUD_PULL_DOWN(45),ADDR_GPIO_GPIPUD1);
