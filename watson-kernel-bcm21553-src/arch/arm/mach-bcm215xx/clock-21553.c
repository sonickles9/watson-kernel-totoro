...
//static int bcm21553_v3d_power_set_rate(struct clk *clk, unsigned long val);
...
/* v3d clk */
static unsigned long bcm21553_v3d_get_rate(struct clk *clk);
...
DEFINE_BRCM_CLK_SIMPLE(v3d, V3D, BRCM_CLK_PTR(arm11), 0, ADDR_SYSCFG_V3D_AHB_CLK_EN,
	0x01);
...
static u32 clk_armahb_reg_to_arm11_freq_mapping[] = {
	...
	FREQ_MHZ(312), // 0x08
	FREQ_MHZ(156), // 0x09
	FREQ_MHZ(156), // 0x0A pll - 312/312 > 468
	FREQ_MHZ(312), // 0x0B pll - 624/312 > 936
	FREQ_MHZ(312), // 0x0C pll - 754/754 > 1131
	FREQ_MHZ(468), // 0x0D pll - 832/832 > 1248
	FREQ_MHZ(468), // 0x0E pll - 910/910 > 1248
	FREQ_MHZ(624) // 0x0F pll - 1040/832 > 1248
};
...
// watson: ahbfast freq is always 1.5x times the ahb freq
// given my observations regarding v3d, would the same apply
// to ahb and ahb_fast? I will test it out: half of the CPU
// can be used for ahb_fast
// response: no, no noticeable performance gain
	FREQ_MHZ(52), // 0x00
	FREQ_MHZ(78), // 0x01
	FREQ_MHZ(52), // 0x02
	FREQ_MHZ(104), // 0x03
	...
	FREQ_MHZ(104), // 0x09
	FREQ_MHZ(104), // 0x0A
	FREQ_MHZ(52), // 0x0B - 364
	FREQ_MHZ(78), // 0x0C pll - 702
	FREQ_MHZ(416), // 0x0D pll - 936
	FREQ_MHZ(468), // 0x0E pll - 1014
	FREQ_MHZ(520) // 0x0F pll - 1144
//	FREQ_MHZ(156)
};

static u32 clk_armahb_reg_to_ahb_freq_mapping[] = {
// the above rule still applies to ahb
	FREQ_MHZ(52), // 0x00
	FREQ_MHZ(78), // 0x01
	FREQ_MHZ(52), // 0x02
	FREQ_MHZ(104), // 0x03
	...
	FREQ_MHZ(156), // 0x09
	FREQ_MHZ(156), // 0x0A
	FREQ_MHZ(39), // 0x0B - 403
	FREQ_MHZ(52), // 0x0C pll - 754
	FREQ_MHZ(260), // 0x0D pll - 1014
	FREQ_MHZ(312), // 0x0E pll - 1092
	FREQ_MHZ(338) // 0x0F pll - 1222
};

static u32 clk_armahb_reg_to_v3d_clk_mapping[] = {
/* for some odd reason, setting the freqs here really high
 * improves performance. My guess is that somehow v3d works
 * mostly using CPU, and the "rate" here would be the amount
 * CPU load (in Hz) that it can use.
 * it seems really silly though.
 */
	FREQ_MHZ(62), // 0x00
	FREQ_MHZ(62), // 0x01
	FREQ_MHZ(62), // 0x02
	...
	FREQ_MHZ(62), // 0x09
	FREQ_MHZ(62), // 0x0A
	FREQ_MHZ(65), // 0x0B - 468
	FREQ_MHZ(78), // 0x0C pll - 832
	FREQ_MHZ(832), // 0x0D pll - 1118
	FREQ_MHZ(910), // 0x0E pll - 1196
	FREQ_MHZ(1040) // 0x0F GONE NUTS
};
...
unsigned long bcm21553_v3d_get_rate(struct clk *clk)
{
	unsigned int mode;
	mode = readl(ADDR_CLKPWR_CLK_ARMAHB_MODE);
	return clk_armahb_reg_to_v3d_clk_mapping[mode];
}
...
unsigned long bcm21553_arm11_get_rate(struct clk *clk)
{
	...
	const u8 apps_pll_mult[] = {12,24,29,32,35,40};
	...
	apps_pll_freq = bcm21553_apps_pll_get_rate();
		return (((apps_pll_freq/1000)/48)*apps_pll_mult[mode-0x0A])*1000;
}
...
int bcm21553_arm11_set_rate(struct clk *clk, unsigned long val)
{
	...
	// 312MHZ
	arm11_freq[0] = (((apps_pll_freq/1000)/48)*12)*1000;
	// 416MHZ
	//arm11_freq[1] = FREQ_MHZ(416);
	// 520MHZ
	//arm11_freq[2] = FREQ_MHZ(520);
	// 624MHZ
	arm11_freq[1] = (((apps_pll_freq/1000)/48)*24)*1000;
	// 754MHZ
	arm11_freq[2] = (((apps_pll_freq/1000)/48)*29)*1000;
	// 832MHZ
	arm11_freq[3] = (((apps_pll_freq/1000)/48)*32)*1000;
	// 910MHZ
	arm11_freq[4] = (((apps_pll_freq/1000)/48)*35)*1000;
	// 1040MHZ
	arm11_freq[5] = (((apps_pll_freq/1000)/48)*40)*1000;
...
unsigned long bcm21553_ahb_get_rate(struct clk *clk)
{
	u32 mode = 0, ahb;
	u32 apps_pll_freq = 0;
	const u8 ahb_mult[] = {2,2,3,10,12,13};
	...
	{
		apps_pll_freq = bcm21553_apps_pll_get_rate();
		ahb = (((apps_pll_freq/1000)/48)*ahb_mult[mode-0x0A])*1000;
	}
...
unsigned long bcm21553_ahb_fast_get_rate(struct clk *clk)
{
	u32 mode = 0, ahb_fast;
	u32 apps_pll_freq = 0;
	const u8 ahb_fast_mult[] = {2,2,3,16,18,20};
	mode = readl(ADDR_CLKPWR_CLK_ARMAHB_MODE);
	...
	{
		apps_pll_freq = bcm21553_apps_pll_get_rate();
		ahb_fast = (((apps_pll_freq/1000)/48)*ahb_fast_mult[mode-0x0A])*1000;
	}
...
unsigned long bcm21553_sdram_get_rate(struct clk *clk)
{
	...
	if (sdram_mode & CLK_SDRAM_SYC_MODE)
	{
		...
		if(mode <= 0xB)
			...
			sdram_clk_speed = apps_pll_freq/16;
	}
	else
	{
...
int bcm21553_v3d_power_enable(struct clk *clk)
{
	...
	u32 ahb_mode;
	int i;
	/* We need to switch to 312Mhz (mode = 0xc) before enabling
	// I will set it to 1248MHz
	...
	ahb_mode = readl(ADDR_CLKPWR_CLK_ARMAHB_MODE);
	//writel(0x0C, ADDR_CLKPWR_CLK_ARMAHB_MODE);
	bcm215xx_set_armahb_mode(0x0F);
	udelay(80);
...
u32 bcm21553_apps_pll_get_rate()
{
	...
	// watson: PLL works by multiples of 26MHz
	// 26 * 48 = 1248
		   ...
		   off_t off, int count, int *eof, void *data)
{
	...
	const u8 arm11_mult[] = {12,24,29,32,35,40};
	u32 apps_pll_freq;
	u32 dsp_clk;
	u32 ahb;
	u32 ahb_fast;
	...
	{
		arm11 = (apps_pll_freq/48)*arm11_mult[mode -0xA];
		cp_clk = apps_pll_freq/cp_clk_div[mode -0xC];
	//	ahb = apps_pll_freq/9;
	...
	if(dsp_mode == 0)
		dsp_clk = 277; /*52 Mhz - only on main pll*/
...
static int __init clk_init(void)
{
	...
	//Watson: it appears we can activate pll on more than 1 mode
	/*To avoid panic in V3D disable call...*/
	if(readl(ADDR_CLKPWR_CLK_ARMAHB_MODE) >= 0x0a)
		bcm215xx_set_appll_enable(1);
