...
extern int Synaptics_Connected;

int Silabs_Connected = 0;
EXPORT_SYMBOL(Silabs_Connected);
...
//	uint8_t buf_tmp[3]={0,0,0};
	...
	if (ret <= 0) 
	{
		printk("[TSP][Silabs][%s] %s\n", __func__,"Failed Silabs i2c");
...
static int silabs_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (device_create_file(firmware_dev, &dev_attr_tsp_id_check   ) < 0)
