...
#include <linux/i2c/cooperve_tsp_gpio.h>
...
//#define __NEW_WORKFUNC__
#define __COOPER_WORKFUNC__
#define __COOPER_CHECKIC__


#define LATEST_FW_VER	0x54
...
#if defined (__COOPER_WORKFUNC__)
...
static DEVICE_ATTR(firmware_ret	, 0664, firmware_ret_show, firmware_ret_store);
/* sys fs */
...
void touch_ctrl_regulator(int on_off)
{
	if(on_off==TOUCH_ON)
	{
			...
			regulator_set_voltage(touch_regulator,3300000,3300000);  // cypress request
...
static void release_all_fingers(void)
{
	...
	{
		if(prev_finger[i] && fingerInfo_bak[prev_finger[i]].prev_status ==FINGER_PRESS)
		{
			fingerInfo_bak[prev_finger[i]].prev_status = FINGER_RELEASE; // force release
	...
	// init fingerInfo_bak
	memset(fingerInfo_bak, 0, sizeof(saved_finger_info_t)*MAX_USING_FINGER_ID);
}
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hrtimer_init(&ts->timer2, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	ts->timer2.function = synaptics_watchdog_timer_func;
	hrtimer_start(&ts->timer2, ktime_set(0, 200000000), HRTIMER_MODE_REL);
...
static int synaptics_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	hrtimer_cancel(&ts->timer2);
	if(!ts->use_irq)
		hrtimer_cancel(&ts->timer);			// for no irq
...
static int synaptics_ts_resume(struct i2c_client *client)
{
	...
	if( g_charger_type != PMU_MUIC_CHGTYP_NONE)
	{
		pre_charger_type = PMU_MUIC_CHGTYP_NONE;
		set_tsp_for_ta_detect(g_charger_type);		
	}
	...
	hrtimer_start(&ts->timer2, ktime_set(0, 500000000), HRTIMER_MODE_REL);
...
static ssize_t firmware_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%03X%02X%02X\n", touch_hw_ver, touch_sw_ver, phone_ver); 
		...
		const char *buf, size_t size)
{
	...
	printk("[TSP] firmware_store  value : %d\n",value);
	if ( value == 1 )
	{
