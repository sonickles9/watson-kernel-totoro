...
#include "tma140_download_totoro.h"
#include "./TMA140_FIRMWARE_TOTORO/Totoro_CY_01_58.h"
...
extern void synaptics_touch_ctrl_regulator(int on_off);

void TchDrv_DownloadVddSetHigh(void)
{
	synaptics_touch_ctrl_regulator(1); /* always on */
}

void TchDrv_DownloadVddSetLow(void)
{
	synaptics_touch_ctrl_regulator(0); /* always off */
}
...
//	struct file *filp;
	...
	if(tsp_special_update == 0)//normal firmware update from phone-binary
	{
		if(HW_ver == 1)
		{
			for(i=0; i<512; i++)
				for(j=0; j<64; j++)
					firmData[i][j] = BinaryData_HW01[i*64 + j];
	...
	else if(tsp_special_update == 1)//special firmware update from t-flash
	{
		...
		if((fd = sys_open("/mnt/sdcard/torino_tsp.hex", O_RDONLY | O_LARGEFILE, 0)) > 0)
		{
...
#if defined(FEATURE_SAMSUNG_SCOTIA)
	if(sv_tch_special_update == 1)
		strcpy(pathName, "/Mount/Mmc/scotia_sgg_test.hex");
	else if(tch_mod_rev == 0x00)
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_00/scotia_sgg_00_lastest.hex");	
	else if(tch_mod_rev == 0x02)
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_02/scotia_sgg_02_lastest.hex");	
	else if(tch_mod_rev == 0x03)//acryl
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_03/scotia_sgg_03_lastest.hex");	
	else if(tch_mod_rev == 0x04)//acryl
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_04/scotia_sgg_04_lastest.hex");	
	else if(tch_mod_rev == 0x06)//acryl,MH3
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_06/scotia_sgg_06_lastest.hex");	
	else if(tch_mod_rev == 0x08)//acryl,MH3,AR
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_08/scotia_sgg_08_lastest.hex");	
	else
		return NO_FIRMWARE_ERROR;
#elif defined(FEATURE_SAMSUNG_PIETRO)
	if(sv_tch_special_update == 1)
		strcpy(pathName, "/Mount/Mmc/pietro_test.hex");
	else if(tch_mod_rev == 0x80)
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_80/pietro_80_lastest.hex");
	else if(tch_mod_rev == 0x81)//acryl
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_81/pietro_81_lastest.hex");
	else if(tch_mod_rev == 0x82)//acryl
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_82/pietro_82_lastest.hex");
	else if(tch_mod_rev == 0x84)//acryl,MH3
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_84/pietro_84_lastest.hex");
	else if(tch_mod_rev == 0x86)//acryl,MH3,AR
		strcpy(pathName, "/SystemFS/Driver/TouchFirmware/TMA340/ModRev_86/pietro_86_lastest.hex");	
	else
		return NO_FIRMWARE_ERROR;
#else
	return NO_FIRMWARE_ERROR;
