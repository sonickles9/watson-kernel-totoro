...
static void melfas_ts_work_func(struct work_struct *work)
{
	int ret = 0, i;
	uint8_t buf[TS_READ_REGS_LEN];
	int touchType=0, touchState =0, touchID=0, posX=0, posY=0, width = 0, strength=0, reportID = 0;
       unsigned long    flags;
	...
	if (ret < 0)
	{
             printk("[TSP] melfas_ts_work_func: i2c failed\n" );
	...
	else 
	{
		...
		if((touchID > MELFAS_MAX_TOUCH-1)||(touchID<0))
		{
            ...
            #ifdef __TOUCH_DEBUG__
             printk("[TSP] x: %d, y: %d\n, 3:%d, 4:%d", g_Mtouch_info[i].posX, g_Mtouch_info[i].posY, g_Mtouch_info[i].strength, g_Mtouch_info[i].width);
		#endif			
				if(g_Mtouch_info[i].strength == 0)
					g_Mtouch_info[i].strength = -1;
			}
