...
module_param(videomemorysize, int, 0444);
...
static int lcdfb_refresh_thread(void *data)
{
	...
	while (wait_event_interruptible_timeout
		...
		if (gLcdRefreshOn == 0) {
			...
			dirtyRect.top = 0;
			dirtyRect.bottom = gLcdDisplayHeight - 1;
			dirtyRect.left = 0;
			dirtyRect.right = gLcdDisplayWidth - 1;
...
//	set_user_nice(gLcdRefreshtask, -10);
...
void lcdfb_cfb_imageblit(struct fb_info *p, const struct fb_image *image)
{
	...
	if (p->state != FBINFO_STATE_RUNNING)
		return;
	/*	The virtual size of the LCD buffer.
	 *	Uusally it is larger than the real size.
	*/
...
int __init lcdfb_init(void)
{
	...
	gLcdFrameBufferSizeBytes =
	    gLcdDisplayWidth * gLcdDisplayHeight * (gLcdBytesPerPixel + 1);
