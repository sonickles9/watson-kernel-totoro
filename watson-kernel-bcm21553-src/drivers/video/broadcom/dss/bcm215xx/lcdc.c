...
{
#if defined(CONFIG_LCD_FRAME_INVERSION_DURING_CALL)
// Watson: I tweaked this to turn on the screen during a call. It may or may not work.
	if (write_lcd_sequence_start_call == 1)
	{
		printk("[LCD] %s, %d, Start Call\n", __func__, __LINE__);
		lcd_send_cmd_sequence(power_on_seq_s6d04h0_boe); /* during call */
...
//		lcd_send_cmd_sequence(power_off_seq); /* Idle */
			       ...
			       LCD_DirtyRect_t * dirtyRect)
{
	...
	/* watson: (apparently) this is where hwa starts to work */
...
{
	LCD_DirtyRect_t rect;
	rect.left = (rect_c->width - rect_c->dx);
	rect.top = (rect_c->height - rect_c->dy);
	rect.right = rect_c->dx;
	rect.bottom = rect_c->dy;
	lcd_dev_dirty_rect(&LCD_device[LCD_main_panel], &rect);
}
...
static int lcd_alloc_fb(LCD_dev_info_t * dev)
{
	...
	/* io_remappable */
	/* DBFrame : */
	/* Our devices allocate framebuffer size HERE: this is the old value
	dev->frame_buffer.sizeInBytes =
	    dev->width * dev->height * 2 * dev->bits_per_pixel / 8; */
	/* ..and this is the NEW value */
	dev->frame_buffer.sizeInBytes =
	    dev->width * (dev->height * 2) * ((dev->bits_per_pixel / 8) + 1);
...
#endif
			// lcd cpu boost!
			arm11 = clk_get(NULL, "arm11");
			if (!arm11)
				pr_info("\nwatson LCD BOOST FAILED");
			else {
			mode = readl(ADDR_CLKPWR_CLK_ARMAHB_MODE); // storing the mode
			pr_info("\nwatson LCD BOOST ON");
...
//			display_black_background();
			...
			lcd_init_panels();
            lcd_dev_dirty_rect(&LCD_device[LCD_main_panel], &init_dirtyRect);
			gInitialized = 1;
			//now back to our usual tasks
...
//	struct lcdc_platform_data_t *pdata = container_of(work, struct lcdc_platform_data_t, work);
	...
	lcd_poweroff_panels();
	lcd_pwr_on_controll(OFF);
	// watson: cutting delay ;p
      msleep(17); //old 50
	lcd_pwr_on_controll(ON);
      	lcd_init_panels();
      lcd_esd_ckeck=1;
      mdelay(167); // old 500
      lcd_dev_dirty_rect(&LCD_device[LCD_main_panel], &init_esd_dirtyRect);
      }
      gpio_direction_output(LCD_DET, 0);
      gpio_direction_input(LCD_DET);
      enable_irq(lcd_esd);
