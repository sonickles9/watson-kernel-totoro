...
#define SET_CURRENT_DELAY			50

struct delayed_work g_delay_workq_set_current;
void usb_change_charging_current(struct work_struct * work);
enum {
...
/*-------------------------------------------------------------------------*/
/**
 * set_usb_interface() - set the interface of the current configuration
 * @function: the function handle to be assigned to the interface
 * @id: the interface id
 * @is_on: 1: set the associated interface, otherwise, reset the associated interface
 *
 */
void set_usb_interface(struct usb_function *function, unsigned id, bool is_on)
{	
		if (is_on)
			c_current->interface[id] = function;					
		else 
			c_current->interface[id] = NULL;					
}

/**
 * set_current_usb_config() - set the the current configuration
 * @is_android: 1: android usb configuration; 0: rndis/ethernet usb configuration
 *
 */
void set_current_usb_config(unsigned char usb_conf)
{	

	switch (usb_conf) {
		case SET_RNDIS_ETH_USB_CONF: 
			c_current = c_rndis;
			break;
		case  SET_ANDROID_USB_CONF:
			c_current = c_android;
			break;
		case SET_ACM_USB_CONF: 
			c_current = c_acm;
			break;				
		default:
			pr_info("Error: USB CONF is not supported...");
	}
...
{
	pr_info("usb_function_set_enabled = %d", enabled);
	f->disabled = !enabled;
...
void rndis_change_charging_current(struct work_struct * work)
{
       dwc_otg_cil_SetPostConfigCurrent();
}
...
static void reset_config(struct usb_composite_dev *cdev)
{
	...
	pr_info("composite: reset config\n");
		...
		const struct usb_ctrlrequest *ctrl, unsigned number)
{
	...
	for (tmp = 0; tmp < MAX_CONFIG_INTERFACES; tmp++) {
		...
		if (result < 0) {			
			pr_info("set_config: interface %d (%s/%p) alt 0 --> %d\n",
					tmp, f->name, f, result);
			reset_config(cdev);
			goto done;
		}
	...
	schedule_delayed_work(&g_delay_workq_set_current, msecs_to_jiffies(SET_CURRENT_DELAY));
		...
		struct usb_configuration *config)
{
	...
	if (config->bConfigurationValue == 1) {
		pr_info("c_android = %x,",(unsigned int) config);
		c_android = config;
	} else if (config->bConfigurationValue == 2) {
		pr_info("c_rndis = %x\n",(unsigned int) config);
		c_rndis = config;
	} else if (config->bConfigurationValue == 3) {
		pr_info("c_acm = %x\n",(unsigned int) config);
		c_acm = config;
	} 
	
	c_current = c_android; //Android ADB/MSC is the default usb configuration
...
)
{
	...
	while (*sp) {
		...
		if (s->language != language &&  language) //For Linux system
			continue;
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_REQ_GET_DESCRIPTOR:		
		if (ctrl->bRequestType != USB_DIR_IN){
			pr_info("ctrl->bRequestType != USB_DIR_IN\n");
			goto unknown;
		}
		...
		case USB_DT_DEVICE_QUALIFIER:			
			if (!gadget_is_dualspeed(gadget))
				break;
			pr_info("USB_DT_DEVICE_QUALIFIER\n");
			device_qual(cdev);
			value = min_t(int, w_length,
				sizeof(struct usb_qualifier_descriptor));
			break;
		case USB_DT_OTHER_SPEED_CONFIG:	
 			pr_info("USB_DT_OTHER_SPEED_CONFIG\n");
			if (!gadget_is_dualspeed(gadget))
				break;
			pr_info("--ok\n");
			/* FALLTHROUGH */
		case USB_DT_CONFIG:		
			pr_info("USB_DT_CONFIG\n");
			value = config_desc(cdev, w_value);
			if (value >= 0)
				value = min(w_length, (u16) value);
			break;
		case USB_DT_STRING:		
			pr_info("USB_DT_STRING\n");
...
unknown:
		...
		/* USB MSC BOT Reset */
		if (ctrl->bRequestType == 0x21 && ctrl->bRequest == 0xFF && ctrl->wValue == 0 && 
			ctrl->wIndex == 0 && ctrl->wLength == 0) { 
			pr_info("USB MSC BOT Reset...\n");
			gadget->ep0->ops->setup_in_status_phase(gadget->ep0, req);
			return 0;
		}
...
send_resp:  
	...
	if (value < 0)
		pr_info("value = %x", value);
	return value;
}

void usb_composite_stop(struct usb_gadget *gadget)
{
	struct usb_composite_dev	*cdev = get_gadget_data(gadget);
	unsigned long			flags;
...
{
	printk(KERN_INFO "usb_composite_register\n");
...
void usb_composite_setup_continue(struct usb_composite_dev *cdev)
{
	...
	if (cdev->delayed_status == 0) {
		WARN(cdev, "%s: Unexpected call\n", __func__);
                ...
                printk(KERN_ALERT "Completing Delayed Status\n");
