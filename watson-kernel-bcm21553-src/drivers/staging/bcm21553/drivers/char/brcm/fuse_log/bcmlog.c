...
//#include <linux/brcm_ulogging_printk.h>
...
#include "brcm_ulogging_bcmlog.h"      
#endif*/
	return ret;
}
...
#ifdef CONFIG_BRCM_CP_CRASH_DUMP
	else if( BCMLOG_CPCRASH_MTD == BCMLOG_GetCpCrashDumpDevice() )
	{
...
void BCMLOG_HandleCpCrashDumpData( const char *buf, int size )
{
	...
	else if( BCMLOG_CPCRASH_BMTT == BCMLOG_GetCpCrashDumpDevice() )
	{       
		brcm_klogging_crashdump( buf, size ) ;
    	}
...
void BCMLOG_LogCPCrashDumpString( const char* inLogString )
{
	...
	if( mttFrameSize > 0 ) 
	{
		...
		{
		   brcm_klogging_crashdump( kbuf_mtt, mttFrameSize ) ;
		}
...
void BCMLOG_HandleCpCrashMemDumpData( const char* inPhysAddr, int size )
{
		...
		p += (WORDS_PER_SIGNAL<<2), currPhysical+=(WORDS_PER_SIGNAL<<2))
	{
		...
		*(pChksum+1) = chksum & 0xFF;
		...
		{
			n = 0;
			snprintf(tmpStr, 255, "CP memory dump done %d of %d bytes. Do not stop logging\n", (int)(p - (UInt32)MemDumpVAddr),size );
