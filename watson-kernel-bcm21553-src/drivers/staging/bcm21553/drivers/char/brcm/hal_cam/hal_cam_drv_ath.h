...
typedef struct {
// Vsync, Hsync, Clock 
    UInt32 sync_mode;                           ///<(default)CAM_SYNC_EXTERNAL:             Sync External or Embedded
    UInt32 vsync_control;                       ///< (default)CAM_SYNC_DEFINES_ACTIVE:      VSYNCS determines active data
    UInt32 vsync_polarity;                      ///< (default)ACTIVE_LOW/ACTIVE_HIGH:       Vsync active  
    UInt32 hsync_control;                       ///< (default)FALSE/TRUE:                   HSYNCS determines active data
    UInt32 hsync_polarity;                      ///< (default)ACTIVE_HIGH/ACTIVE_LOW:       Hsync active 
    UInt32 data_clock_sample;                   ///< (default)RISING_EDGE/FALLING_EDGE:     Pixel Clock Sample edge
    UInt32 bus_width;                           ///< (default)CAM_BITWIDTH_8:               Camera bus width
    UInt32 data_shift;                          ///< (default)0:                            data shift (+) left shift  (-) right shift
    UInt32 field_mode;                          ///< (default)CAM_FIELD_H_V:                field calculated
    UInt32 data_intr_enable;                    ///< CAM_INTERRUPT_t                        (default)CAM_INT_FRAME_END:
    UInt32 pkt_intr_enable;                     ///< CAM_INTERRUPT_t                        (default)CAM_INT_FRAME_END:
} CamIntfConfig_CCIR656_st_t;   

/** Sensor CCP/CSI Interface Configuration */
typedef struct {
    UInt32 input_mode;                          ///< CAM_INPUT_MODE_T:
    UInt32 clk_mode;                            ///< CAM_CLOCK_MODE_T:  
    UInt32 encoder;                             ///< CAM_ENCODER_T
    UInt32 enc_predictor;                       ///< CAM_PREDICTOR_MODE_t
    UInt32 decoder;                             ///< CAM_DECODER_T
    UInt32 dec_predictor;                       ///< CAM_PREDICTOR_MODE_t
    UInt32 sub_channel;                         ///< CAM_CHAN_SEL_t
    UInt32 term_sel;                            ///< 
    UInt32 bus_width;                           ///< CAM_BITWIDTH_t
    UInt32 emb_data_type;                       ///< CAM_DATA_TYPE_T
    UInt32 emb_data_channel;                    ///< CAM_DATA_TYPE_T
    UInt32 short_pkt;                           ///< CAM_DATA_TYPE_T
    UInt32 short_pkt_chan;                      ///< CAM_DATA_TYPE_T
    UInt32 data_intr_enable;                    ///< CAM_INTERRUPT_t                        (default)CAM_INT_FRAME_END:
    UInt32 pkt_intr_enable;                     ///< CAM_INTERRUPT_t                        (default)CAM_INT_FRAME_END:
} CamIntfConfig_CCP_CSI_st_t; 
...
typedef struct {
 //   HAL_CAM_IntConfigCaps_st_t          *sensor_config_caps;
    HAL_CAM_ConfigCaps_st_t          *sensor_config_caps;
    CamIntfConfig_CCIR656_st_t          *sensor_config_ccir656;
	CamIntfConfig_CCP_CSI_st_t          *sensor_config_ccp_csi;
...
//#else
	...
	CAM_Sensor_Supported_Params_t		*sensor_default_setting;
...
//HAL_CAM_Result_en_t CamacqExtWriteI2c(UInt8 *pucValue, UInt8 ucSize ); //BYKIM_CAMACQ
...
struct sens_methods {
    ...
    HAL_CAM_ResolutionSize_st_t *sensor_size );
    HAL_CAM_Result_en_t (*DRV_SetVideoCaptureMode)(CamImageSize_t image_resolution,CamDataFmt_t image_format,CamSensorSelect_t sensor,CamMode_t mode);
