...
static HAL_CAM_Result_en_t Init_tassve_sensor(CamSensorSelect_t sensor)
{
	HAL_CAM_Result_en_t result = HAL_CAM_SUCCESS;
	...
	if(iRet<0)
	{
      ...
      //reset settings	
       ...
       Drv_DTPmode= NULL;
        ...
        printk(KERN_ERR"Init_tassve_sensor end \n");
...
HAL_CAM_Result_en_t CAMDRV_Wakeup(CamSensorSelect_t sensor)
{
	...
	result = Init_tassve_sensor(sensor);
	//Call totoro_init() before
	printk(KERN_ERR"Init_tassve_sensor :%d \r\n",result);
	return result;
}
...

{
	...
	if(currentLux == Low_Lux)
	{
       	printk(KERN_INFO"CAMDRV_CfgStillnThumbCapture(): current Lux = LOW \r\n");
		if (gv_Nightshot_mode)
		{
	       	printk(KERN_INFO"CAMDRV_CfgStillnThumbCapture(): current Lux = NIGHT LOW \r\n");
	...
	else if(currentLux == High_Lux)
	{
       	printk(KERN_INFO"CAMDRV_CfgStillnThumbCapture(): current Lux = HIGH \r\n");
		pstSensor->m_pstAPIs->WriteDirectSensorData( pstSensor, CAMACQ_SENSORDATA_CAPTURE_OUTDOOR );
	}
	else
	{
       	printk(KERN_INFO"CAMDRV_CfgStillnThumbCapture(): current Lux = NORMAL \r\n");
...
UInt8  CAMDRV_CheckEXP(UInt8 mode)
{
	...
	U32 reg_main_local_checkEXP[4]={0xFCFCD000,0x002C7000,0x002E2A14,0xFFFFFFFF};
	U16 addr=0x0F12;
	U8 ReadShutterSpeed1[2]={0,0};
	U8 ReadShutterSpeed2[2]={0,0};
	...
	{
		CamacqExtWriteI2cLists( pstSensor->m_pI2cClient,reg_main_local_checkEXP,0);
		CamacqExtReadI2c( pstSensor->m_pI2cClient, addr,2, &ReadShutterSpeed1,2);
		CamacqExtReadI2c( pstSensor->m_pI2cClient, addr,2, &ReadShutterSpeed2,2);
		printk(KERN_INFO"ReadShutterSpeed1[0] = 0x%x,  ReadShutterSpeed1[1] = 0x%x \r\n",ReadShutterSpeed1[0],ReadShutterSpeed1[1]);
		printk(KERN_INFO"ReadShutterSpeed2[0] = 0x%x,  ReadShutterSpeed2[1] = 0x%x \r\n",ReadShutterSpeed2[0],ReadShutterSpeed2[1]);

		gv_checkEXPtime = ((ReadShutterSpeed2[0]<<24)|(ReadShutterSpeed2[1]<<16)|(ReadShutterSpeed1[0]<<8)|ReadShutterSpeed1[1])/400;
		printk(KERN_INFO"gv_checkEXPtime = %d \r\n",gv_checkEXPtime);
		
		return gv_checkEXPtime;
	}
	else
		return NULL;
...
void  CAMDRV_CheckISO(void)
{
	...
	U32 sensor= 0;//Rear Camera
	U32 reg_main_local_checkISO[4]={0xFCFCD000,0x002C7000,0x002E2A18,0xFFFFFFFF};
	U16 addr = 0x0F12;
	U8 ReadISO[2]={0,0};
	U32 ReadGain;
	float ReadGain_float=0;
	U32 ReadGain_int=0;
	...
	CamacqExtWriteI2cLists( pstSensor->m_pI2cClient,reg_main_local_checkISO,0);
	CamacqExtReadI2c( pstSensor->m_pI2cClient, addr,2, &ReadISO,2);
	printk(KERN_INFO"CAMDRV_CheckISO() : ReadISO[0] = 0x%x,  ReadISO[2] = 0x%x \r\n",ReadISO[0],ReadISO[1]);

	ReadGain = ((ReadISO[0]<<8)|ReadISO[1]);
	printk(KERN_INFO"CAMDRV_CheckISO() : ReadGain = %ld\r\n",ReadGain);
       ReadGain_int = ( ReadGain*1000 / 256);
	printk(KERN_INFO"CAMDRV_CheckISO() : ReadGain = %ld\r\n",ReadGain_int);

#if 1
        if((ReadGain_int> 1000)&&(ReadGain_int<1899))
        {
            gv_isoSpeedRatings= 50;
        }
        else if ((ReadGain_int > 1900)&&(ReadGain_int<2299))
        {
            gv_isoSpeedRatings= 100;
        }
        else if ((ReadGain_int > 2300)&&(ReadGain_int<2799))
        {
            gv_isoSpeedRatings= 200;
        }
        else if (ReadGain_int > 2800)
        {
...
HAL_CAM_Result_en_t CAMDRV_GetSensorValuesForEXIF( CAM_Sensor_Values_For_Exif_t *exif_parm,CamSensorSelect_t sensor)
{
	...
	if(gv_checkEXPtime < 1000)
	{
		gv_checkEXPtime = 1000/gv_checkEXPtime;
     		sprintf(aString, "1/%d", gv_checkEXPtime); 
	}
	else
	{
		gv_checkEXPtime = gv_checkEXPtime/1000;
             sprintf(aString, "%d", gv_checkEXPtime); 
	}
    ...
    strcpy(exif_parm->softwareUsed,"S5570IXXKH3");//NOT_USED 
...
{ 
	...
	U32 reg_main_local_checklux[4] = {0xFCFCD000,	0x002C7000,	0x002E2A3C,0xFFFFFFFF};
	U16 addr=0x0F12;
	U8 msb_lsb[2]={0,0};
	U8 msb_lsb2[2]={0,0};
	U16 currentLux;
	...
	CamacqExtWriteI2cLists(pstSensor->m_pI2cClient, reg_main_local_checklux,0);
	CamacqExtReadI2c( pstSensor->m_pI2cClient, addr,2, msb_lsb,2);	
	CamacqExtReadI2c( pstSensor->m_pI2cClient, addr,2, msb_lsb2,2);	
	printk(KERN_INFO"CAMDRV_GetCurrentLux : msb_lsb[0] =0x%x, msb_lsb[1]=0x%x \r\n", msb_lsb[0],msb_lsb[1]);
	printk(KERN_INFO"CAMDRV_GetCurrentLux : msb_lsb2[0] =0x%x, msb_lsb2[1]=0x%x \r\n", msb_lsb2[0],msb_lsb2[1]);

	currentLux = (msb_lsb2[0]<<24)|(msb_lsb2[1]<<16)|(msb_lsb[0]<<8)|msb_lsb[1];
	printk(KERN_INFO"CAMDRV_GetCurrentLux : currentLux=0x%x \r\n",currentLux);

	if(currentLux>0xFFFE)	//HIGH LUX
	{
		return High_Lux;
	}
	else if(currentLux<0x0020)	//LOW LUX
	{
		return Low_Lux;
	}
	else	//NORMAL LUX
