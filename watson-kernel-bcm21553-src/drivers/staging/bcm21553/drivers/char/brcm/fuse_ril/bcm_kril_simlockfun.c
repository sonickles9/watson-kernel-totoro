...
static Boolean GetSimlockNvdata(SIMLOCK_NVDATA_t *simlock_nvdata)
{
    ...
    else
    {
          ...
          (simlock_nvdata->check_sum != check_sum))
        {
             ...
             */
            KRIL_DEBUG(DBG_ERROR,"simlock_nvdata Error!!! simlock_init_flag:0x%lX simlock_nvdata->check_sum:%d check_sum:%d\n",
                simlock_nvdata->simlock_init_flag, simlock_nvdata->check_sum, check_sum);
...
static SIMLock_CodeFile_t* GetSimlockData(void)
{
    if (IsSIMSecureEnable())
    {
        ...
        // Veify the SIM lock data check sum.
        simlock_data = (SIMLock_CodeFile_t *)simlock_data_ptr;
        simDataSize = simlock_data->numNetworkLocks * sizeof(SIMLock_NetworkCode_t) +
                      simlock_data->numProviderLocks * sizeof(SIMLock_ProviderCode_t) +
                      simlock_data->numNetSubsetLocks * sizeof(SIMLock_NetSubSetCode_t) +
                      simlock_data->numCorpLocks * sizeof(SIMLock_CorporateCode_t);
        ...
        {
            KRIL_DEBUG(DBG_ERROR,"SIM lock data chksum not match chksum:%d simlock_data->chksum:%d Error!!!\n",chksum, simlock_data->chksum);
            freeSimLockdata(simlock_data);
            kernel_power_off();
        }
