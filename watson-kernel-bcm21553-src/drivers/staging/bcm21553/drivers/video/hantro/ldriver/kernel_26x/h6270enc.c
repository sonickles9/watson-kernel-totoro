...
static struct clk *hx_codec_island_clk = NULL;

#ifdef CONFIG_CPU_FREQ_GOV_BCM21553
static struct cpufreq_client_desc *hantro_enc_client;
#endif
...
static long h6270enc_ioctl(struct file *filp,
...
#ifdef CONFIG_CPU_FREQ_GOV_BCM21553
	printk("disabling DVFS as hantro enc is being used\n");
//	cpufreq_bcm_dvfs_disable(hantro_enc_client);
#endif
...
//#ifdef CONFIG_CPU_FREQ_GOV_BCM21553
//	cpufreq_bcm_dvfs_e(hantro_enc_client);
//#endif
...
int __init h6270enc_init(void)
{
	...
	hx_codec_island_clk = clk_get(NULL,BCM_CLK_VCODEC_POWER_STR_ID);

#ifdef CONFIG_CPU_FREQ_GOV_BCM21553
	hantro_enc_client = cpufreq_bcm_client_get("hantro_enc");
	if (!hantro_enc_client) {
...
//	cpufreq_bcm_dvfs_disable(hantro_enc_client);
	...
	if (result < 0) {
		...
		clk_disable(hx_codec_island_clk);
//#ifdef CONFIG_CPU_FREQ_GOV_BCM21553
//		cpufreq_bcm_dvfs_enable(hantro_enc_client);
//#endif
		goto err;
	}
