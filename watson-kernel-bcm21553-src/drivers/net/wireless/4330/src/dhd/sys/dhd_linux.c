...
extern void cmd_set_pm_qos(const char *);
struct cpufreq_client_desc *wlan_enc_client=NULL;
EXPORT_SYMBOL(wlan_enc_client);
#include <linux/pm_qos_params.h>
static struct pm_qos_request_list *wlan_driver_pm_qos_req=NULL;
#endif
...
int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	else
	{
		if (!value && !dhd->in_suspend)
		{
			DHD_TRACE(("%s: ELSE DISABLE BROADCASR \n", __FUNCTION__));
			dhd_set_packet_filter(0, dhd);
		}
	}
...
#if 1
#ifdef CONFIG_CPU_FREQ_GOV_BCM21553
	DHD_ERROR(("%s: TRY  Start DVFS WLAN as Client Registration DVFS Ptr=%x \n", __FUNCTION__,wlan_enc_client));
...
//			cpufreq_bcm_dvfs_disable(wlan_enc_client); //Shri
			DHD_ERROR(("%s: DVFS Client Registred and Disabled for WLAN usage DVFS Ptr=%x \n", __FUNCTION__,wlan_enc_client));
			}
		}
#endif
#endif
...
//	cpufreq_bcm_dvfs_enable(wlan_enc_client); //Shri
	...
	DHD_ERROR(("%s: WLAN sets DVFS Enable before Release DVFS Ptr=%x \n", __FUNCTION__,wlan_enc_client));

	cpufreq_bcm_client_put(	wlan_enc_client); //Shri


	DHD_ERROR(("%s: WLAN as DVFS Client Released Registration DVFS Ptr=%x \n", __FUNCTION__,wlan_enc_client));
		wlan_enc_client=NULL;

	}
	else
	DHD_ERROR(("%s: ******DVFS client ID was NULL***** \n", __FUNCTION__));
#endif
